...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for var in self.domains:
            ...
            for val in self.domains[var]:
                # If value length does not match variable length, add it to remove
                if len(val) != var_len:
                    to_remove.add(val)

            # Remove all invalid values from variable domain
            self.domains[var] = self.domains[var] - to_remove

    def overlap(self, x, y, val_x, val_y):
            """
    ...
    def revise(self, x, y):
        ...
        for val_x in self.domains[x]:
            ...
            for val_y in self.domains[y]:
                if val_x != val_y and self.overlap(x, y, val_x, val_y):
                    consistent = True
                    break
    ...
    def ac3(self, arcs=None):
        ...
        while arcs:
            ...
            # Revise domain x wrt y:
            if self.revise(var_x, var_y):
                # If domain x is empty after revision, return no solution:
    ...
    def consistent(self, assignment):
        ...
        for var_x in assignment:
            ...
            # If the assigned word is already used:
            if val_x in used_variables:
                return False
            used_variables.append(val_x)

            # Check if variable length is correct:
            ...
            for var_y in self.crossword.neighbors(var_x):
                if var_y in assignment:
                    ...
                    if not self.overlap(var_x, var_y, val_x, val_y):
                        return False
