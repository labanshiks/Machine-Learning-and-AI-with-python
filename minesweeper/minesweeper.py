...
class Sentence():
    ...
    def known_mines(self):
        ...
        # If count of mines is equal to number of cells and greater than 0 then all cells are mines:
    ...
    def mark_mine(self, cell):
        ...
        # If cell is in the sentence, remove it and decrease the count by 1
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Create set for unknown cells:
        new_sentence_cells = set()

        # Creating a new sentence by looping over all cells within one row and column
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
        ...
        #  Marking guaranteed mines and safes, and additional ones from new knowledge:
    ...
    def make_random_move(self):
        ...
        # Calculate probability of any cell being a mine:
        mines_left = MINES - len(self.mines)
        cells_left = (self.height * self.width) - (len(self.moves_made) + len(self.mines))

        # If no cells are left that are acceptable moves, return no move possible
        if cells_left == 0:
            return None

        mines_prob = mines_left / cells_left

        # Get list of all possible moves that are not mines
        for i in range(0, self.height):
            for j in range(0, self.width):
                if (i, j) not in self.moves_made and (i, j) not in self.mines:
                    moves[(i, j)] = mines_prob

        # If no moves have been made then any move is available:
