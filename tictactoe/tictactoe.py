...
def player(board):
    ...
    """
    # Checking the board and determine the next player:
...
def result(board, action):
    ...
    if i not in [0, 1, 2] or j not in [0, 1, 2]:
      ...
      raise InvalidActionError(action, board, 'Invalid action on selected tile: ')

    # Making a deep copy of the board: 
    board_copy = deepcopy(board)
    # Updating with the current player's move:
...
def winner(board):
    ...
    # Otherwise there is no current winner
    return None
...
def terminal(board):
    ...
    """
    # Game is over if it is a winning board or there are no more possible moves:
...
def minimax(board):
    ...
    def min_player(board, best_max = -10):
      """ Helper function to minimise score for 'X' player """
